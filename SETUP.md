...
| 5   | wolffi.sh zone → **Security → Security rules** (rate limiting)     | Rule `relay-connect-brake`: expression `(http.host eq "relay.wolffi.sh")`, per-IP, `10000` requests / `10 s`, action **Block** for `10 s` | **Not deployed — break-glass playbook.** Deliberately removed 2026-08-02: no hardening until a real spike happens. If one does, recreate with exactly these values (~1 min; free plan fixes period and block duration at 10 s, threshold is the only knob)      |
