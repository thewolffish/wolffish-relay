...
| 5   | wolffi.sh zone → **Security → Security rules** (rate limiting)     | Rule `relay-connect-brake`: expression `(http.host eq "relay.wolffi.sh")`, per-IP, `10000` requests / `10 s`, action **Block** for `10 s` | **Deployed 2026-08-02, Active** — a pure connection-storm brake at the most permissive threshold the plan allows; unreachable by legitimate use (upgrades are single requests; long-lived sockets aren't re-counted)                                            |
| 6   | wolffi.sh zone → **Security → Settings → Bot traffic**             | Bot Fight Mode                                                                                                                            | **Off** (verified 2026-08-02; the `JS Detections: On` sub-setting is dormant while the master toggle is off — confirmed by the live page carrying no injected script and setting no cookies). Bot challenges would break native (non-browser) WebSocket clients |
| 7   | wolffi.sh zone → **Security → Settings**                           | Challenge behavior for `relay.wolffi.sh`                                                                                                  | No managed challenge on this hostname (same reason as #6). If the zone ever runs strict security, add a skip rule scoped to the hostname                                                                                                                        |
...
| 9   | wolffi.sh zone → **Network → Network Error Logging**               | NEL ("browser reports that show end user connectivity")                                                                                   | **Off** (switched off 2026-08-02) — with it on, Cloudflare adds `report-to`/`nel` headers asking visitors' browsers to send connectivity telemetry to `a.nel.cloudflare.com`. Verified absent from live responses after the toggle                              |
