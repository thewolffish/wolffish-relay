...
3. **Model-led only, desktop-stamped identity.** Notifications exist because the desktop's model called a tool; the relay adds no triggers of its own. `notify` is accepted only from the `host` socket, `register_push`/`notification_ack`/`set_badge`/`unregister_push` only from `guest`, and a notify's `phoneId` must match a registration on this pairing — reject, never reroute.
